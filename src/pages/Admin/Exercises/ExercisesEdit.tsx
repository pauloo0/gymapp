...
export default function ExercisesEdit() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true)
      try {
        const [resExercise, resEquipment, resBodyparts] = await Promise.all([
          axios.get(`${apiUrl}/exercises/${exercise_id}`),
        ...
        if (!resExercise) {
          setErrorMessage('Não encontrei o exercício')
          return
        }
        ...
        const currentExercise: Exercise = resExercise.data.exercises
        form.reset({
          ...currentExercise,
          equipment_id: currentExercise.equipment.id,
          bodypart_id: currentExercise.bodyparts.id,
        })
      } catch (error) {
  ...
  return (
    <main className='min-h-[calc(100vh_-_64px)] pb-[80px]'>
      <h1 className='mb-6 text-xl'>Editar exercício</h1>
